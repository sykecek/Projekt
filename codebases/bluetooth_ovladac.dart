...
class BluetoothController extends GetxController {
  ...
  var showPairingGuide = true.obs; ///určuje, zda se má zobrazit návod (text + obrázek)
  ...
  void _showBluetoothOffSnackbar() {
    /// Pomocná metoda: zobrazí uživateli informaci, že Bluetooth není zapnutý.
    /// Používáme Get.snackbar, protože už ho v projektu používáš a je rychlý pro oznámení.
    Get.snackbar(
      'Bluetooth není zapnutý', ///titulek snackbaru
      'Zapněte Bluetooth v telefonu a zkuste to znovu.', ///text snakbaru - jednoduché vysvětlení co má uživatel udělat
      snackPosition: SnackPosition.BOTTOM, ///pozice snakbaru na spodní části obrazovky
      backgroundColor: Get.theme.colorScheme.error, ///barva pozadí - chybová barva z aktuálního tématu
      colorText: Get.theme.colorScheme.onError, ///barva textu - kontrastní k chybovému pozadí
      duration: const Duration(seconds: 4), ///doba zobrazení
      maxWidth: 320, ///omezí šířku snakbaru (lepší čitelnost na širších displejích)
    );
  }
  ...
  Future<void> scanDevices() async { ///asynchronní metoda pro skenování spárovaných Bluetooth zařízení. ///vrací Future<void> (nevrací žádnou hodnotu).
    final permissionsOk = await ensureBluetoothPermissions(); ///zajistí, že jsou udělena potřebná Bluetooth oprávnění před pokračováním.
    if (!permissionsOk) { ///pokud uživatel oprávnění nedal, dál nepokračujeme
      return; ///ukončí skenování bez změny UI
    }

    final enabled = await bluetooth.isEnabled; ///zjistí, jestli je Bluetooth adaptér v systému zapnutý (true/false)
    if (enabled != true) { ///pokud je BT vypnutý (nebo se nepodařilo stav zjistit)
      _showBluetoothOffSnackbar(); ///zobrazí oznámení uživateli
      return; // BT je vypnutý -> nech návod viditelný (text + obrázek zůstane)
    }

    showPairingGuide.value = false; ///BT je zapnutý -> schovej návod a uvolni místo pro seznam
    ...
    try {
      ...
      showPairingGuide.value = true; // při chybě vrať návod zpět (aby uživatel viděl instrukce)
      Get.snackbar(
        'Chyba skenování', ///titulek - chyba
        'Nepodařilo se načíst spárovaná zařízení.', ///text - stručné vysvětlení
        snackPosition: SnackPosition.BOTTOM, ///pozice snakbaru
        backgroundColor: Get.theme.colorScheme.error, ///chybová barva pozadí
        colorText: Get.theme.colorScheme.onError, ///kontrastní text
        duration: const Duration(seconds: 4), ///doba zobrazení
        maxWidth: 320, ///maximální šířka
      );
    } finally {
  ...
  Future<void> connectToDevice(BluetoothDevice device) async { ///asynchronní metoda pro připojení k zadanému Bluetooth zařízení. ///vrací Future<void> (nevrací žádnou hodnotu). ///parametr device typu BluetoothDevice představuje zařízení, ke kterému se chceme připojit.
    final permissionsOk = await ensureBluetoothPermissions(); ///ověří/poptá si potřebná oprávnění pro Bluetooth (scan/connect)
    if (!permissionsOk) { ///pokud oprávnění nejsou udělena
      return; ///nepokračuj v připojování
    }

    final enabled = await bluetooth.isEnabled; ///zjistí, zda je Bluetooth zapnutý
    if (enabled != true) { ///když je vypnutý
      _showBluetoothOffSnackbar(); ///zobraz informaci uživateli
      return; ///nezkoušej připojení (to by stejně selhalo)
    }
